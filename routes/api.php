...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::post('/units/import-floor-plans', [UnitController::class, 'importFloorPlans']);
});
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::patch('/bookings/{booking}/customers', [BookingController::class, 'updateCustomers']);
...
Route::get('/updatesquares', function (Request $request) {
    $unitNumbers = [
        "101",
        "102",
        "103",
    ...
    foreach ($unitNumbers as $index => $unitNo) {
        $square = $internalSquares[$index];
        $square2 = $externalSquares[$index];

        \App\Models\Unit::where('unit_no', $unitNo)
            ->update(['internal_square' => $square, 'external_square' => $square2]);
    }
});
